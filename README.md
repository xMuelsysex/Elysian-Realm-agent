...
`agents[].persona` accepts either a plain description string (legacy) or a **structured character contract** (recommended) with facets injected into conversation, life-narrative, and nightly-reflection prompts: `identity`, `personality`, `values`, `speechStyle`, `boundaries` (OOC red lines), `behaviorTraits`, and `exampleLines` (in-voice sample lines), plus temperament knobs: `baseline` (affect decay target; e.g. Elysia upbeat 0.35/0.4, Mobius composed 0.0/0.2), `affectModifiers` (per-event response multipliers — Elysia treasures praise 1.3×, Mobius shrugs it off 0.4× but remembers criticism 1.6×), and `emotionResponsiveness` (conversation-emotion blend weight, default 0.1). A plain string is rendered as the legacy single `Persona:` section.

Behavior adapts to mood: `routines` may carry a `mood: low|neutral|high` preference, and the host picks the matching routine each tick (a low-Elysia stays home instead of the garden). Optional `plotScript` feeds scripted plot events per period automatically. `user.profile` tells the agent who the participant is (injected as `About 主人: ...`), and the conversation prompt carries the relationship trajectory (`Relationship trajectory: ... moved from X to Y`) so replies feel the bond evolving.
