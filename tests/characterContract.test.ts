...
test("life narrative prompt injects the current emotional state", () => {
  const { user } = buildLifeNarrativeMessages({
    ...
    affect: {
      ...
      arousal: 0.8,
      emotionLabels: {
        neutral: 0, admiration: 0, amusement: 0, anger: 0.6, annoyance: 0, approval: 0,
        caring: 0, confusion: 0, curiosity: 0, desire: 0, disappointment: 0, disapproval: 0,
        ...
        relief: 0, remorse: 0, sadness: 0, surprise: 0,
      },
      baseline: { valence: 0.2, arousal: 0.3 },
      updatedAt: "2026-07-26T08:00:00.000Z",
    },
  });
  assert.match(user, /Your current emotional state: tense and unsettled/);
  assert.match(user, /anger \(0\.60\)/);
});
